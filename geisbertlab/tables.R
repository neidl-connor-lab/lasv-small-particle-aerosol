...
clinical <- df$WT %>%
            ...
            select(Tattoo, DPI, Fever, Hypothermia) %>%
            # melt to long
            reshape2::melt(id.vars=c("Tattoo", "DPI"),
                           variable.name="Change",
                           value.name="Present") %>%
            filter(Present) %>%
            # compile by group
            group_by(Tattoo, Change) %>%
...
clinical <- df$observations %>%
            ...
            select(Tattoo, DPI, `decreased appetite`, anorexia) %>%
            # melt to long
            reshape2::melt(id.vars=c("Tattoo", "DPI"),
...
# survival
endpoint <- df$animals %>%
            select(Tattoo, `Death DPI`, Outcome) %>%
...
clinical <- clinical %>%
            ...
            reshape2::melt(id.vars=c("Tattoo", "Outcome"),
...
clinical <- clinical %>%
            ...
            left_join(illness, by=c("Tattoo", "Outcome")) %>%
...
baseline <- df$hematology %>%
            filter(DPI==0) %>%
            select(Tattoo, Leukocytes, Lymphocytes, Monocytes, Erythrocytes,
                   Platelets, Neutrophils, Eosinophils, Basophils) %>%
            reshape2::melt(id.vars="Tattoo",
...
clinpath <- df$hematology %>%
            ...
            left_join(baseline, by=c("Tattoo", "Celltype")) %>%
            mutate(Fraction=(Concentration/Baseline)) %>%
            mutate(Penia=(Fraction <= 0.65),
                   Tosis=(Fraction >= 2)) %>%
            filter(Penia | Tosis) %>%
            select(Tattoo, DPI, Celltype, Penia, Tosis) %>%
            # reshape again and modulate to get names
            reshape2::melt(id.vars=c("Tattoo", "DPI", "Celltype"),
                           variable.name="Change",
                           value.name="Present") %>%
            filter(Present) %>%
...
baseline <- df$chemistry %>%
            left_join(df$hematology,
                      by=c("Tattoo", "DPI")) %>%
            filter(DPI==0) %>%
            select(Tattoo, `Glucose (GLU)`, Hematocrit, Hemoglobin, Erythrocytes,
...
chem <- df$chemistry %>%
        left_join(df$hematology,
                  by=c("Tattoo", "DPI")) %>%
        select(Tattoo, DPI, unique(baseline$Analyte)) %>%
        # melt for baseline calculations
        reshape2::melt(id.vars=c("Tattoo", "DPI"),
                       variable.name="Analyte",
                       value.name="Concentration") %>%
        left_join(baseline, by=c("Tattoo", "Analyte")) %>%
        mutate(Change=(Concentration/Baseline)) %>%
        select(Tattoo, DPI, Analyte, Change) %>%
        # expand for terminology calculations
        reshape2::dcast(Tattoo + DPI ~ Analyte, value.var="Change")
clinpath <- chem %>%
...
clinpath <- chem %>%
            ...
            group_by(Tattoo, Analyte, Arrows) %>%
            summarise(DPI=paste(DPI, collapse=", "),
                      .groups="drop") %>%
            # combine with arrows
...
df$animals %>%
  ...
  select(Tattoo, Sex, `Dose (PFU)`) %>%
  # add clinical illness
  left_join(clinical, by="Tattoo") %>%
  # add clinical pathology
  left_join(clinpath, by="Tattoo") %>%
  # arrange by dose
